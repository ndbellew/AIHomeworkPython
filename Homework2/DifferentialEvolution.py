...
def printBoard(self, individual):
...
def differentialEvolution(self):
    for generation in range(self.iterations):
        for individual in range(self.populationSize):
            ...
            a, b, c = self.selectThreeRandomVectors(self.population, individual)
            ...
            if self.solutionFound(self.population):
                break
    return self.bestSolution(self.population)
